...
redbird = pygame.image.load("./res/photos/red-bird3.png").convert_alpha()
background_image = pygame.image.load(
    "./res/photos/background3.png").convert_alpha()
sling_image = pygame.image.load("./res/photos/sling-3.png").convert_alpha()
full_sprite = pygame.image.load("./res/photos/full-sprite.png").convert_alpha()
...
pig_image = pygame.transform.scale(
    cropped_image, (30, 30))  # (30, 30) resulting height and width of pig
...
static_lines_first = [
    pymunk.Segment(static_floor_body, (0.0, 060.0), (1200.0, 060.0), 0.0)
]
static_lines_second = [
    pymunk.Segment(static_floor_body, (1200.0, 060.0), (1200.0, 800.0), 0.0)
]
...
def sling_action():
    ...
    if mouse_distance > rope_length:
        ...
        second_pos = (uv_1 * highest_length + initial_x_sling,
                      uv_2 * highest_length + initial_y_sling
                      )  # c urrent position
        pygame.draw.line(screen, (255, 0, 0), (next_x_sling, next_y_sling),
                         second_pos, 5)  # catapult rope
        screen.blit(redbird, first_pos)
        pygame.draw.line(screen, (255, 0, 0),
                         (initial_x_sling, initial_y_sling), second_pos,
                         5)  # ANOTHER SIDE of catapult
    else:
        ...
        pygame.draw.line(screen, (0, 0, 0), (initial_x_sling, initial_y_sling),
                         third_pos, 5)
    # Angle of impulse
...
def post_solve_bird_pig(arbiter, space_obj, _):
    ...
    radius = 30
    pygame.draw.circle(screen, (255, 0, 0), bird_position, radius,
                       4)  # screen => pygame surface
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if (pygame.mouse.get_pressed()[0] and mouse_x_pos > 100
                and mouse_x_pos < 250 and mouse_y_pos > 370
                and mouse_y_pos < 550):
            mouse_pressed = True
        if (event.type == pygame.MOUSEBUTTONUP and event.button == 1
            ...
            if level.number_of_birds > 0:
                ...
                if mouse_x_pos < initial_x_sling + 5:
                    bird = RoundBird(mouse_distance, angle, x_initial,
                                     y_initial, space_obj)
    ...
    if mouse_pressed and level.number_of_birds > 0:
        ...
        if time.time(
        ) * 1000 - time_of_release > 300 and level.number_of_birds > 0:
    ...
    for Each_pig in total_pig:
        ...
        pig_rotated_img = pygame.transform.rotate(
            pig_image,
            angle_degrees)  # small random rotation within wooden frame
    ...
    for x in range(2):
        space_obj.step(time_step_change
                       )  # make two updates per frame for better stability
